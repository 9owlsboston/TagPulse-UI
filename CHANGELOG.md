...
### Added
- **Sprint 28 G2 — UI CRUD audit.** [docs/ui-crud-audit-sprint28.md](docs/ui-crud-audit-sprint28.md) is the page-by-page CRUD coverage table for every route in [src/App.tsx](src/App.tsx). Confirms the Phase G gap list (G3 sites/zones edit+delete, G4 device edit, G5 device bulk re-assign, G6 telemetry-models edit, G7 tenant map-config inline editor, G8 asset attribute edit), records the gaps that are intentionally out-of-scope (alerts/audit-logs/stock-movements as `n/a`), and notes one deferral (integration delivery retry → Sprint 29, gated on backend `dead_letter_events.source` routing). Companion to TagPulse roadmap Phase G entry.
